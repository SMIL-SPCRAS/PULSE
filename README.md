...
- `my_code/` — research and training code for the multi-task learning experiments described below.
...
The remainder of this README describes the research and training code located in `my_code/`. For the developed application itself, see the source code in `app/`.

## Requirements

The training code in `my_code/` requires Python 3.11 or newer because the configuration is loaded with the standard-library `tomllib` module.
...
The versions used in the reference training environment are pinned in `my_code/requirements.txt`.
...
Separate train, validation, and test embedding/CSV pairs are configured in `my_code/config.toml`.
...
The separator and field indices can be changed in `my_code/config.toml`.

## Configuration

All training and experiment settings are stored in `my_code/config.toml`.
...
Before running an experiment, replace all `/path/to/...` placeholders in the `[paths]` section of `my_code/config.toml` with the actual dataset locations.
...
`my_code/run_training.py` always reads the `[grid]` section from `my_code/config.toml` and performs grid search over all listed values.
...
After editing `my_code/config.toml`, run:
...
When training is launched from `my_code/` as shown above, checkpoints are written by default to:
